...
def dashboard(request):
    ...
    avg_upload = Device.objects.filter(status=Device.Status.ONLINE).aggregate(
        avg=Sum('upload_speed')
    )['avg'] or 0
    avg_download = Device.objects.filter(status=Device.Status.ONLINE).aggregate(
        avg=Sum('download_speed')
    )['avg'] or 0
    ...
    top_consumers = Device.objects.filter(
        status=Device.Status.ONLINE
    ).order_by('-download_speed')[:5]
    
    # Get device groups with counts
    device_groups = DeviceGroup.objects.annotate(
...
def device_list(request):
    ...
    # Apply sorting
    valid_sort_fields = ['name', '-name', 'last_seen', '-last_seen', 
